...
public class ResDownloadConfig {
    ...
    private static List<MotionDLModel> addModelList(String category, String[] motionList) {
        ...
        for (int i = 0; i < motionList.length; i++) {
            list.add(new MotionDLModel(
                    category, motionList[i], MOTION_RES_DOWNLOAD_PREFIX + motionList[i] + ".zip"));
        }
        return list;
    }

    private static final String[] MotionResGan = new String[] {"video_bubblegum"};

    private static final String[] MotionResSegment =
            new String[] {"video_empty_segmentation", "video_segmentation_transparent_bg",
                    "video_segmentation_blur_45", "video_segmentation_blur_75"};
    ...
    private static final String[] MotionResHand = new String[] {"video_sakuragirl"};
