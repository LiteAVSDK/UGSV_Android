...
public class BitmapUtil {
    ...
    private static String compressImage(Context context, String imgPath) {
        ...
        LogUtils.d(TAG,
                "compressImage ,sampleSize = " + sampleSize + " , degree = " + degree + "  imgPath= " + imgPath);
        if (sampleSize == 1) {
            if (degree == 0) {  //不需要裁剪，也不需要旋转的情况
                return imgPath;
            } else {   //不需要裁剪，但是需要旋转
     ...
     * @param callBack 回调接口
     */
    public static void compressImage(@NonNull Context context, @NonNull String imgPath,
                                     @NonNull CompressImageCallBack callBack) {
    ...
    private static String saveImageToFile(Context context, String imgPath, Bitmap bitmap) {
        ...
        String filePath = directory + File.separator + FileUtil.getMD5(imgPath)
                + BitmapUtil.getNameByCompressFormat(compressFormat);
