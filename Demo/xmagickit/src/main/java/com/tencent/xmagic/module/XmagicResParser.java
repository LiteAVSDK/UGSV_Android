 ...
 * 基础美颜美颜：美白、磨皮、红润
 * 画面调整：对比度、饱和度、锐化
 * 高级美颜(人像属性识别能力)：大眼、窄脸、瘦脸（自然、女神、英俊）、V
 * 脸、下巴、短脸、脸型、发际线、亮眼、
 ...
 * Advanced beauty (portrait attribute recognition ability): big eyes, thin face (natural, goddess,
 * handsome) Filter effects
 * ************************************************ ************************************
 * A1-02
 ...
 * Covers all functions of A1-01
 * 2D special effect capability, 2D special effect material resource package
 * Advanced beauty (portrait attribute recognition ability): increase narrow face/chin/hairline/slim
 * nose
 * ************************************************ ************************************
 * A1-04
 * Covers all functions of A1-01
 ...
 * (natural, goddess, handsome), V face, chin, short face, face shape, hairline, bright eyes, eye
 * distance, eye corners, thin nose , nose wing, nose position, white teeth, wrinkle removal,
 * nasolabial wrinkle removal, eye bag removal, mouth shape, lip thickness, lipstick, blush,
 * three-dimensional Filter effects 2D special effect capability, 2D special effect material
 * resource package 3D special effect capability, 3D special effect material resource package
 * Resource packs for beauty abilities and beauty effects
 * ************************************************ ************************************
 * S1-02
...
public class XmagicResParser {
     ...
     * 2. copyRes(Context context) Copy the resource file in the asset to the path set in the first
     * step
     * 3. Call the parseRes() method to classify the resources
     * 4. Then you can use the methods of XmagicPanelView and XmagicPanelDataManager.getInstance()
     * classes
     */
    ...
    public static void copyRes(Context context) {
        SharedPreferences sharedPreferences =
                context.getSharedPreferences("xmagic_settings", Context.MODE_PRIVATE);
    ...
    private static boolean copyResource(Context context) {
        ...
        for (String path : new String[] {"lut"}) {
            boolean result = FileUtil.copyAssets(
                    context, path, sResPath + "light_material" + File.separator + path);
            if (!result) {
                Log.d(TAG,
                        "copyRes: fail,path=" + path + ",new path=" + sResPath + "light_material"
                                + File.separator + path);
                return false;
            }
        }
    ...
    public static void parseRes(Context context) {
        ...
        if (!file.exists() || list == null || list.length == 0) {
            throw new IllegalStateException(
                    "resource dir not found or empty, call XmagicResParser.copyRes first.");
        }
        ...
        //        XmagicPanelDataManager.getInstance().addAllDataItem(UICategory.BODY_BEAUTY,
        //        bodyBeautyList); parseBodyBeauty(context, bodyBeautyList);
    ...
    private static void pareBeauty(Context context, List<XmagicUIProperty<?>> beautyList) {
        ...
        XmagicUIProperty<?> beautyProperty =
                new XmagicUIProperty<>(UICategory.KV, null, 0, null, null);
        ...
        XmagicUIProperty<?> xmagicUIProperty = new XmagicUIProperty<>(UICategory.BEAUTY,
                ...
                BeautyConstant.BEAUTY_WHITEN, new XmagicPropertyValues(0, 100, 30, 0, 1));
        XmagicPanelDataManager.getInstance().getSelectedItems().put(
                xmagicUIProperty.uiCategory.getDescription(), xmagicUIProperty);
        beautyList.add(xmagicUIProperty);
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_smooth_label), R.mipmap.beauty_smooth,
                BeautyConstant.BEAUTY_SMOOTH, new XmagicPropertyValues(0, 100, 50, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_ruddy_label), R.mipmap.beauty_ruddy,
                BeautyConstant.BEAUTY_ROSY, new XmagicPropertyValues(0, 100, 20, 0, 2)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.image_contrast_label), R.mipmap.image_contrast,
                BeautyConstant.BEAUTY_CONTRAST, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.image_saturation_label), R.mipmap.image_saturation,
                BeautyConstant.BEAUTY_SATURATION, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.image_sharpen_label), R.mipmap.image_sharpen,
                BeautyConstant.BEAUTY_CLEAR, new XmagicPropertyValues(0, 100, 0, 0, 2)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
        ...
        XmagicUIProperty thinFace =
                new XmagicUIProperty(thinFaceLabel, R.mipmap.beauty_thin_face, UICategory.BEAUTY);
        beautyList.add(thinFace);
        List<XmagicUIProperty<?>> slList = new ArrayList<>();
        slList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_thin_face1_label), "nature", null,
                R.mipmap.beauty_thin_face1, BeautyConstant.BEAUTY_FACE_NATURE,
                new XmagicPropertyValues(0, 100, 30, 0, 1), thinFaceLabel));
        slList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_thin_face2_label), "femaleGod", null,
                R.mipmap.beauty_thin_face2, BeautyConstant.BEAUTY_FACE_GODNESS,
                new XmagicPropertyValues(0, 100, 0, 0, 1), thinFaceLabel));
        slList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_thin_face3_label), "maleGod", null,
                R.mipmap.beauty_thin_face3, BeautyConstant.BEAUTY_FACE_MALE_GOD,
                new XmagicPropertyValues(0, 100, 0, 0, 1), thinFaceLabel));
        thinFace.xmagicUIPropertyList = slList;

        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_v_face_label), R.mipmap.beauty_v_face,
                BeautyConstant.BEAUTY_FACE_V, new XmagicPropertyValues(0, 100, 30, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_narrow_face_label), R.mipmap.beauty_narrow_face,
                BeautyConstant.BEAUTY_FACE_THIN, new XmagicPropertyValues(0, 100, 0, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_short_face_label), R.mipmap.beauty_short_face,
                BeautyConstant.BEAUTY_FACE_SHORT, new XmagicPropertyValues(0, 100, 0, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_basic_face_label), R.mipmap.beauty_basic_face,
                BeautyConstant.BEAUTY_FACE_BASIC, new XmagicPropertyValues(0, 100, 0, 0, 1)));
        ...
        for (String ids : lipsResPathNames.keySet()) {
            itemLipsPropertys.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                    lipsResPathNames.get(ids), lipId, effDirs + effs.get(ids), R.mipmap.beauty_lips,
                    BeautyConstant.BEAUTY_MOUTH_LIPSTICK,
                    new XmagicPropertyValues(0, 100, 50, 0, 1), lipLabel));
        }
        XmagicUIProperty itemLips =
                new XmagicUIProperty<>(lipLabel, R.mipmap.beauty_lips, UICategory.BEAUTY);
        ...
        redcheeksResPathNames.put(
                "saihong_jianyue.png", context.getString(R.string.beauty_redcheeks1_label));
        redcheeksResPathNames.put(
                "saihong_shengxia.png", context.getString(R.string.beauty_redcheeks2_label));
        redcheeksResPathNames.put(
                "saihong_haixiu.png", context.getString(R.string.beauty_redcheeks3_label));
        redcheeksResPathNames.put(
                "saihong_chengshu.png", context.getString(R.string.beauty_redcheeks4_label));
        redcheeksResPathNames.put(
                "saihong_queban.png", context.getString(R.string.beauty_redcheeks5_label));
        ...
        for (String ids : redcheeksResPathNames.keySet()) {
            itemRedcheekPropertys.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                    redcheeksResPathNames.get(ids), redcheekId, effDirs + effs.get(ids),
                    R.mipmap.beauty_redcheeks, BeautyConstant.BEAUTY_FACE_RED_CHEEK,
                    new XmagicPropertyValues(0, 100, 50, 0, 1), redcheeksLabel));
        }
        ...
        for (String ids : litisResPathNames.keySet()) {
            liTiItems.add(new XmagicUIProperty<>(UICategory.BEAUTY, litisResPathNames.get(ids),
                    "beauty.softLight.softLightMask", effDirs + effs.get(ids), R.mipmap.beauty_liti,
                    BeautyConstant.BEAUTY_FACE_SOFTLIGHT,
                    new XmagicPropertyValues(0, 100, 50, 0, 1), liTiLabel));
        }
        XmagicUIProperty itemLiTi =
                new XmagicUIProperty(liTiLabel, R.mipmap.beauty_liti, UICategory.BEAUTY);
        itemLiTi.xmagicUIPropertyList = liTiItems;
        beautyList.add(itemLiTi);

        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_thin_cheek_label), R.mipmap.beauty_thin_cheek,
                BeautyConstant.BEAUTY_FACE_THIN_CHEEKBONE,
                new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_chin_label), R.mipmap.beauty_chin,
                BeautyConstant.BEAUTY_FACE_THIN_CHIN,
                new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_forehead_label), R.mipmap.beauty_forehead,
                BeautyConstant.BEAUTY_FACE_FOREHEAD,
                new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_eye_lighten_label), R.mipmap.beauty_eye_lighten,
                BeautyConstant.BEAUTY_EYE_LIGHTEN, new XmagicPropertyValues(0, 100, 30, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_eye_distance_label), R.mipmap.beauty_eye_distance,
                BeautyConstant.BEAUTY_EYE_DISTANCE, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_eye_angle_label), R.mipmap.beauty_eye_angle,
                BeautyConstant.BEAUTY_EYE_ANGLE, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_thin_nose_label), R.mipmap.beauty_thin_nose,
                BeautyConstant.BEAUTY_NOSE_THIN, new XmagicPropertyValues(0, 100, 0, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_nose_wing_label), R.mipmap.beauty_nose_wing,
                BeautyConstant.BEAUTY_NOSE_WING, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_nose_position_label),
                R.mipmap.beauty_nose_position, BeautyConstant.BEAUTY_NOSE_HEIGHT,
                new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_tooth_beauty_label), R.mipmap.beauty_tooth_beauty,
                BeautyConstant.BEAUTY_TOOTH_WHITEN, new XmagicPropertyValues(0, 100, 0, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
        ...
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                ...
                new XmagicPropertyValues(0, 100, 0, 0, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_mouth_size_label), R.mipmap.beauty_mouth_size,
                BeautyConstant.BEAUTY_MOUTH_SIZE, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
        beautyList.add(new XmagicUIProperty<>(UICategory.BEAUTY,
                context.getString(R.string.beauty_mouth_height_label), R.mipmap.beauty_mouth_height,
                BeautyConstant.BEAUTY_MOUTH_HEIGHT, new XmagicPropertyValues(-100, 100, 0, -1, 1)));
    }
    ...
    private static void parseLutProperty(Context context, List<XmagicUIProperty<?>> lutList) {
        ...
        LutData[] lutDatas = {new LutData(context.getString(R.string.lut_item1_label),
                                      "baixi_lf.png", R.mipmap.filter_baizhi),
                new LutData(context.getString(R.string.lut_item2_label), "ziran_lf.png",
                        R.mipmap.filter_ziran),
                new LutData(context.getString(R.string.lut_item3_label), "moren_lf.png",
                        R.mipmap.filter_chulian),
                new LutData(context.getString(R.string.lut_item4_label), "xindong_lf.png",
                        R.mipmap.filter_xindong),
                new LutData(context.getString(R.string.lut_item5_label), "dongjing_lf.png",
                        R.mipmap.filter_dongjing)};

        for (LutData lutData : lutDatas) {
            if (exists(lutDir + lutData.id)) {
                lutList.add(new XmagicUIProperty<XmagicPropertyValues>(UICategory.LUT, lutData.name,
                        lutData.id, lutDir + lutData.id, lutData.resourceId, null,
    ...
    private static void parseMotionData(Context context, List<XmagicUIProperty<?>> motionsData) {
        ...
        XmagicUIProperty item2dMotions = new XmagicUIProperty<>(
                context.getString(R.string.motion_2d_label), R.mipmap.motion_2d, UICategory.MOTION);
        item2dMotions.xmagicUIPropertyList =
                new ArrayList<XmagicUIProperty<XmagicPropertyValues>>();
        parseMotion(context, motion2dResPath, map, context.getString(R.string.motion_3d_label),
        ...
        XmagicUIProperty item3dMotions = new XmagicUIProperty<>(
                context.getString(R.string.motion_3d_label), R.mipmap.motion_3d, UICategory.MOTION);
        item3dMotions.xmagicUIPropertyList =
                new ArrayList<XmagicUIProperty<XmagicPropertyValues>>();
        ...
        XmagicUIProperty itemHandMotions =
                new XmagicUIProperty<>(context.getString(R.string.motion_hand_label),
                        R.mipmap.motion_hand, UICategory.MOTION);
        itemHandMotions.xmagicUIPropertyList =
                new ArrayList<XmagicUIProperty<XmagicPropertyValues>>();
        ...
        XmagicUIProperty itemGanMotions =
                new XmagicUIProperty<>(context.getString(R.string.motion_gan_label),
    ...
    private static void parseMotion(Context context, String filePath,
            LinkedHashMap<String, String> converter, String name, MotionType motionType,
            ...
            for (File file : ganMotionFiles) {
                if (file.isDirectory()) {
                    ...
                    XmagicUIProperty uiProperty =
                            createMotionUiProperty(id, converter, filePath, name);
        ...
        for (MotionDLModel motionDLModel : motionDLModels) {
            XmagicUIProperty uiProperty =
                    createMotionUiProperty(motionDLModel.getName(), converter, filePath, name);
            uiProperty.thumbImagePath =
                    MotionDLUtils.getIconUrlByName(context, motionDLModel.getName());
    ...
    private static XmagicUIProperty createMotionUiProperty(String id,
            LinkedHashMap<String, String> nameConverter, String resourcePath,
            String rootDisplayName) {
        String name = nameConverter.get(id);
        String materialPath = resourcePath + id;
        name = (name != null ? name : id);
        return new XmagicUIProperty<>(
                UICategory.MOTION, name, id, materialPath, 0, null, null, rootDisplayName);
    }
    ...
    private static void parseMakeUpData(Context context, List<XmagicUIProperty<?>> makeupData) {
        ...
        List<MotionDLModel> motionDLModelList =
                MotionDLUtils.getMotionsByType(context, MotionType.MotionResMakeup);
        ...
        if (makeupFiles != null) {
            for (File file : makeupFiles) {
                if (file.isDirectory()) {
                    ...
                    name = (name != null ? name : id);
                    XmagicPropertyValues xmagicPropertyValues =
                            new XmagicPropertyValues(0, 100, 60, 0, 1);
                    XmagicUIProperty xmagicUIProperty = new XmagicUIProperty<>(UICategory.MAKEUP,
                            name, id, materialPath, 0, "makeup.strength", xmagicPropertyValues);
        ...
        for (MotionDLModel motionDLModel : motionDLModelList) {
            ...
            XmagicUIProperty<XmagicPropertyValues> uiProperty =
                    new XmagicUIProperty<>(UICategory.MAKEUP, name, id, makeupResPath + id, 0,
    ...
    private static void parseSegData(Context context, List<XmagicUIProperty<?>> segListData) {
        ...
        List<MotionDLModel> motionDLModelList =
                MotionDLUtils.getMotionsByType(context, MotionType.MotionResSegment);
        ...
        LinkedHashMap<String, String> map =
                getMaterialDataByStr(context.getString(R.string.segmentation_resource_str));
            ...
            for (File file : segmentMotionFiles) {
                if (file.isDirectory()) {
                    ...
                    if (id.equals("video_empty_segmentation")) {
                        segListData.add(new XmagicUIProperty<>(UICategory.SEGMENTATION,
                                context.getString(R.string.segmentation_custom_label), id,
                                materialPath, R.mipmap.segmentation_formulate, "", null));
                    } else {
        ...
        for (MotionDLModel motionDLModel : motionDLModelList) {
            ...
            XmagicUIProperty<XmagicPropertyValues> uiProperty = new XmagicUIProperty<>(
                    UICategory.SEGMENTATION, name, id, segmentMotionResPath + id, 0, null, null);
            uiProperty.thumbImagePath =
                    MotionDLUtils.getIconUrlByName(context, motionDLModel.getName());
            uiProperty.dlModel = motionDLModel;
            segListData.add(uiProperty);
    ...
    private static String getTemplateImgPath(String motionName) {
        return sResPath + "MotionRes" + File.separator + motionName + File.separator
                + "template.png";
    }
    ...
    private static void ensureResPathAlreadySet() {
        if (TextUtils.isEmpty(sResPath)) {
            throw new IllegalStateException(
                    "resource path not set, call XmagicResParser.setResPath() first.");
        }
    }
