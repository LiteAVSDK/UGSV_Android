...
public class TCRegisterActivity extends Activity {
    ...
    private void attemptNormalRegist(String username, String password, String passwordVerify) {
        if (!mCbAgreeProtocol.isChecked()) {
            Toast.makeText(TCRegisterActivity.this, getString(R.string.login_protocol_tip),
                         Toast.LENGTH_SHORT)
                    .show();
            return;
        }
        ...
        if (!UserInfoUtil.isUsernameVaild(username)) {
            showRegistError(getResources().getString(
                    R.string.tc_register_activity_attempt_normal_regist_username_does_not_meet_specifications));
            return;
        }
        if (!UserInfoUtil.isPasswordValid(password)) {
            showPasswordVerifyError(getResources().getString(
                    R.string.tc_register_activity_attempt_normal_regist_password_length_error));
            return;
        }
        if (!password.equals(passwordVerify)) {
            showPasswordVerifyError(getResources().getString(
                    R.string.tc_register_activity_attempt_normal_regist_password_verify_error));
            return;
        }
        if (!NetworkUtil.isNetworkAvailable(this)) {
            ToastUtil.toastShortMessage(getResources().getString(
                    R.string.tc_register_activity_attempt_normal_regist_no_network_connection));
            return;
        }
        ...
        tcLoginMgr.register(username, password, new TCUserMgr.Callback() {
            ...
            public void onSuccess(JSONObject data) {
                showToast(getResources().getString(
                        R.string.tc_register_activity_register_callback_register_successfully));
                tcLoginMgr.uploadLogs(LogReport.ELK_ACTION_REGISTER, username,
                        TCUserMgr.SUCCESS_CODE, "注册成功", new Callback() {
                            @Override
                ...
                tcLoginMgr.login(username, password, new TCUserMgr.Callback() {
                    ...
                    public void onSuccess(JSONObject data) {
                        showToast(getResources().getString(
                                R.string.tc_register_activity_register_callback_auto_login_successfully));
                        jumpToHomeActivity();
                    }
                    ...
                    public void onFailure(int code, final String msg) {
                        showToast(getResources().getString(
                                R.string.tc_register_activity_register_callback_auto_login_failed));
            ...
            public void onFailure(int code, final String msg) {
                ...
                if (code == 610) {
                    errorMsg = getResources().getString(
                            R.string.tc_register_activity_register_callback_username_format_error);
                } else if (code == 611) {
                    errorMsg = getResources().getString(
                            R.string.tc_register_activity_register_callback_password_format_error);
                } else if (code == 612) {
                    errorMsg = getResources().getString(
                            R.string.tc_register_activity_register_callback_username_already_exists);
                }
                tcLoginMgr.uploadLogs(
                        LogReport.ELK_ACTION_REGISTER, username, code, errorMsg, new Callback() {
                            @Override
                            public void onFailure(Call call, IOException e) {
                                Log.d(TAG, "uploadLogs onFailure");
                            }
                ...
                showToast(getResources().getString(
                                  R.string.tc_register_activity_register_callback_register_failed)
                        + errorMsg);
                showOnLoadingInUIThread(false);
            }
        });
