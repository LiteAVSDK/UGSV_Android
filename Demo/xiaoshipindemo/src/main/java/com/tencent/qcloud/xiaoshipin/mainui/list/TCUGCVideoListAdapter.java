 ...
 * 列表项数据格式: TCLiveInfo
 */
public class TCUGCVideoListAdapter
        extends BaseRecyclerAdapter<TCUGCVideoListAdapter.VideoVideoHolder> {
    ...
    public void onBindVH(VideoVideoHolder holder, int position) {
        ...
        if (TextUtils.isEmpty(cover)
                || data.review_status == TCVideoInfo.REVIEW_STATUS_PORN) { // 涉黄的图片不显示
        ...
        TCUtils.showPicWithUrl(
                holder.itemView.getContext(), holder.ivAvatar, data.headpic, R.drawable.face);
        //昵称
        ...
        if (data.review_status == TCVideoInfo.REVIEW_STATUS_NORMAL) {
            holder.reviewStatus.setText(mContext.getResources().getString(
                    R.string.tc_ugc_video_list_adapter_video_state_normal));
        } else if (data.review_status == TCVideoInfo.REVIEW_STATUS_NOT_REVIEW) { // 审核中
            holder.reviewStatus.setText(mContext.getResources().getString(
                    R.string.tc_ugc_video_list_adapter_video_state_in_audit));
        } else if (data.review_status == TCVideoInfo.REVIEW_STATUS_PORN) { // 涉黄
            holder.reviewStatus.setText(mContext.getResources().getString(
                    R.string.tc_ugc_video_list_adapter_video_state_pornographic));
        }
    }
    ...
    public VideoVideoHolder onCreateVH(ViewGroup parent, int viewType) {
        View itemView =
                LayoutInflater.from(parent.getContext()).inflate(R.layout.listview_ugc_item, null);
    ...
    public String generateTimeStr(long timestamp) {
        String result =
                mContext.getResources().getString(R.string.tc_ugc_video_list_adapter_just_now);
        ...
        if (timeDistanceinSec >= 60 && timeDistanceinSec < 3600) {
            ...
            if (min > 1) {
                result = String.format("%d ", (timeDistanceinSec) / 60)
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_mins_ago);
            } else {
                result = String.format("%d ", (timeDistanceinSec) / 60)
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_min_ago);
            }
            ...
            if (hour > 1) {
                result = String.format("%d ", (timeDistanceinSec) / 3600)
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_hours_ago);
            } else {
                result = String.format("%d ", (timeDistanceinSec) / 3600)
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_hour_ago);
            }
            ...
            if (day > 1) {
                result = String.format("%d ", (timeDistanceinSec) / (3600 * 24))
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_days_ago);
            } else {
                result = String.format("%d ", (timeDistanceinSec) / (3600 * 24))
                        + mContext.getResources().getString(
                                R.string.tc_ugc_video_list_adapter_day_ago);
            }
        }
        return result;
    }
