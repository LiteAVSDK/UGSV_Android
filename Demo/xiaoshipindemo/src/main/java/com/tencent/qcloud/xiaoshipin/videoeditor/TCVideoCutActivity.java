...
public class TCVideoCutActivity extends FragmentActivity {
    ...
    private IVideoCutKit.OnCutListener mOnCutListener = new IVideoCutKit.OnCutListener() {
        ...
        public void onCutterCompleted(UGCKitResult ugcKitResult) {
            ...
            if (ugcKitResult.errorCode == 0) {
                ...
                ToastUtil.toastShortMessage("cut video failed. error code:" + ugcKitResult.errorCode
                        + ",desc msg:" + ugcKitResult.descMsg);
            }
        }
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        String path;
        //由于Android
        // 10上在使用ContentResolver.openFileDescriptor的时候会卡主，系统问题（https://issuetracker.google.com/issues/141496793）
        //所以修改为在Android 10以及之后通过uri转为绝对路径，并且在manifest文件中配置
        // android:requestLegacyExternalStorage="true"
        if (Build.VERSION.SDK_INT >= 29) {
            path = TextUtils.isEmpty(inVideoUri) ? inVideoPath
                                                 : URIConvert.getFilePathByUri(inVideoUri);
        } else {
            path = inVideoPath;
        }
