...
public class PermissionManager {
    ...
    public PermissionManager(Context context, PermissionType type) {
        ...
        sharedPreferenceUtils =
                new SharedPreferenceUtils(mContext, SHARED_PREFERENCE_FILE_NAME_PERMISSION);
    }
    ...
    private void checkOrShowPermissionIntroductionDialog() {
        ...
        if (mPermissionType.equals(PermissionType.STORAGE)) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                oldTime = (long) sharedPreferenceUtils.getSharedPreference(
                        SHARED_PREFERENCE_KEY_STORAGE, 0L);
                if (mContext.checkSelfPermission(PERMISSION_STORAGE)
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                oldTime = (long) sharedPreferenceUtils.getSharedPreference(
                        SHARED_PREFERENCE_KEY_AUDIO, 0L);
                if (mContext.checkSelfPermission(PERMISSION_AUDIO)
                        != PackageManager.PERMISSION_GRANTED) {
                    if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                oldTime = (long) sharedPreferenceUtils.getSharedPreference(
                        SHARED_PREFERENCE_KEY_CAMERA, 0L);
                if (mContext.checkSelfPermission(PERMISSION_CAMERA)
                        != PackageManager.PERMISSION_GRANTED) {
                    if ((now - oldTime) >= FORTY_EIGHT_HOURS) {
    ...
    private PermissionIntroductionDialog createDialog() {
        ...
        if (mPermissionType.equals(PermissionType.STORAGE)) {
            mDialog = new PermissionIntroductionDialog(
                    mContext.getString(R.string.permission_introduction_write_title),
                    mContext.getString(R.string.permission_introduction_write),
                    PermissionIntroductionDialog.DialogPosition.TOP);
        } else if (mPermissionType.equals(PermissionType.AUDIO)) {
            mDialog = new PermissionIntroductionDialog(
                    mContext.getString(R.string.permission_introduction_audio_title),
                    mContext.getString(R.string.permission_introduction_audio),
                    PermissionIntroductionDialog.DialogPosition.TOP);
        } else if (mPermissionType.equals(PermissionType.CAMERA)) {
            mDialog = new PermissionIntroductionDialog(
                    mContext.getString(R.string.permission_introduction_camera_title),
    ...
    public interface OnStoragePermissionGrantedListener { void onStoragePermissionGranted(); }
    ...
    public interface OnCameraPermissionGrantedListener { void onCameraPermissionGranted(); }
    ...
    public interface OnAudioPermissionGrantedListener { void onAudioPermissionGranted(); }
