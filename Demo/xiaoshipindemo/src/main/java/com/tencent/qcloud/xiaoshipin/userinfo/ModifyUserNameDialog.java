...
public class ModifyUserNameDialog extends BottomSheetDialog {
    ...
    private void setProfile() {
        ...
        if (!userName.matches(reg)) {
            mTextInputTips.setTextColor(
                    mContext.getResources().getColor(R.color.app_color_input_no_match));
            return;
        }
        mTextInputTips.setTextColor(
                mContext.getResources().getColor(R.color.app_color_input_normal));
        TCUserMgr.getInstance().setNickName(userName, new TCUserMgr.Callback() {
            ...
            public void onFailure(int code, String msg) {
                ToastUtils.showLong(
                        mContext.getString(R.string.app_toast_failed_to_set_username, msg));
            }
        });
    }

    public interface ModifySuccessListener { void onSuccess(); }
}