...
public class AudioFocusManager {
    ...
    public AudioFocusManager(
            @NonNull Context context, @NonNull OnAudioFocusChangeListener listener) {
     ...
     * 获取音频焦点。
     *
     * @param streamType   受焦点请求影响的主要音频流类型。该参数通常是
     *         <code>AudioManager.STREAM_MUSIC。更多音频
     *                     流类型，请查看
     ...
     *                     （API level
     * 19）：表示短暂获取音频焦点，在此期间，其他任何应用程序或系统组件均不应播放任何内容。
     *                     更多内容，https://developer.android.google.cn/reference/android/media/AudioManager
     * @return AudioManager.AUDIOFOCUS_REQUEST_GRANTED（申请成功）或者
     *         AudioManager.AUDIOFOCUS_REQUEST_FAILED（申请失败）
     */
    public int requestAudioFocus(int streamType, int durationHint) {
        ...
        return mAudioManager.requestAudioFocus(mAudioFocusChangeListener, streamType, durationHint);
    }
