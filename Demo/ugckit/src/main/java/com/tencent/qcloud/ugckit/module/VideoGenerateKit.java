...
public class VideoGenerateKit extends BaseGenerateKit implements TXVideoGenerateListener {
    ...
    public void startGenerate() {
        ...
        if (editer != null) {
            ...
            if (mTailWaterMarkConfig != null) {
                editer.setTailWaterMark(mTailWaterMarkConfig.tailwatermark,
                        mTailWaterMarkConfig.rect, mTailWaterMarkConfig.duration);
            }

            switch (mVideoResolution) {
                case TXVideoEditConstants.VIDEO_COMPRESSED_360P:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_360P, mVideoOutputPath);
                    break;
                case TXVideoEditConstants.VIDEO_COMPRESSED_480P:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_480P, mVideoOutputPath);
                    break;
                case TXVideoEditConstants.VIDEO_COMPRESSED_540P:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_540P, mVideoOutputPath);
                    break;
                case TXVideoEditConstants.VIDEO_COMPRESSED_720P:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_720P, mVideoOutputPath);
                    break;
                case TXVideoEditConstants.VIDEO_COMPRESSED_1080P:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_1080P, mVideoOutputPath);
                    break;
                default:
                    editer.generateVideo(
                            TXVideoEditConstants.VIDEO_COMPRESSED_720P, mVideoOutputPath);
    ...
    public void stopGenerate() {
        ...
        if (mCurrentState == PlayState.STATE_GENERATE) {
            ToastUtil.toastShortMessage(UGCKit.getAppContext().getResources().getString(
                    R.string.ugckit_video_editer_activity_cancel_video_generation));
    ...
    public void addTailWaterMark() {
        ...
        Bitmap tailWaterMarkBitmap = BitmapFactory.decodeResource(
                UGCKit.getAppContext().getResources(), R.drawable.ugckit_tcloud_logo);
        float widthHeightRatio =
                tailWaterMarkBitmap.getWidth() / (float) tailWaterMarkBitmap.getHeight();

        TXVideoEditConstants.TXRect rect = new TXVideoEditConstants.TXRect();
        // 归一化的片尾水印，这里设置了一个固定值，水印占屏幕宽度的0.25。
        rect.width = 0.25f;
        // 后面根据实际图片的宽高比，计算出对应缩放后的图片的宽度：txRect.width * videoInfo.width
        // 和高度：txRect.width * videoInfo.width /
        // widthHeightRatio，然后计算出水印放中间时的左上角位置
    ...
    private void saveAndUpdate(@NonNull TXVideoEditConstants.TXGenerateResult result) {
        if (mSaveToDCIM) {
            ...
            AlbumSaver.getInstance(UGCKit.getAppContext())
                    .setOutputProfile(mVideoOutputPath, duration, mCoverPath);
