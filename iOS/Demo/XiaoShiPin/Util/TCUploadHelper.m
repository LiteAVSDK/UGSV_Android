...
- (void)getCOSSign:(void (^)(int errCode))handler
{
    [[TCLoginModel sharedInstance] getCosSign:^(int errCode, NSString *msg, NSDictionary *resultDict) {
        ...
        else
        {
            ...
            QCloudAuthentationHeadV5Creator *creator = self->_creator;
            if (creator == nil) {
                creator = [[QCloudAuthentationHeadV5Creator alloc] initWithSignKey:[[TCUserInfoModel sharedInstance] getUserProfile].secretId  signKey:strSignKey keyTime:strKeyTime];
            } else {
                [creator setSignKey:[[TCUserInfoModel sharedInstance] getUserProfile].secretId signKey:strSignKey keyTime:strKeyTime];
            }
            handler(errCode);
        }
    }];
