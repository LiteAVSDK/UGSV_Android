...
#pragma mark -
@interface TCBeautyPanel () <TCMenuViewDataSource, TCMenuViewDelegate, NSURLSessionDelegate, NSURLSessionDownloadDelegate> {
    TCMenuView *                                    _menu;
...
+ (instancetype)beautyPanelWithFrame:(CGRect)frame actionPerformer:(id<TCBeautyPanelActionPerformer>)actionPerformer {
    TCBeautyPanel *panel = [[TCBeautyPanel alloc] initWithFrame:frame theme:nil actionPerformer:actionPerformer];
    return panel;
}
...
- (instancetype)initWithFrame:(CGRect)frame theme:(id<TCBeautyPanelThemeProtocol>)theme actionPerformer:(id<TCBeautyPanelActionPerformer>)actionPerformer {
...
- (void)resetAndApplyValues {
    ...
    if (self.actionPerformer) {
        ...
        [performer setWhitenessLevel:self.whiteLevel];
        [performer setRuddyLevel:self.ruddyLevel];
...
- (void)_generateMenuItems {
    ...
    NSArray * (^makeMenuItemsFromPituMotions)(NSArray<TCPituMotion *> *motions) = ^(NSArray<TCPituMotion *> *motions) {
        ...
        for (TCPituMotion *motion in motions) {
            TCBeautyPanelItem *item = [TCBeautyPanelItem itemWithTitle:motion.name icon:[self->_theme imageNamed:motion.identifier]];
...
- (void)layoutSubviews {
    ...
    _menu.frame                 = CGRectMake(0, CGRectGetHeight(self.bounds) - MenuHeight - _bottomOffset, CGRectGetWidth(self.bounds), MenuHeight);
    self.slider.frame           = CGRectMake(BeautyViewMargin * 4, CGRectGetMinY(_menu.frame) - BeautyViewMargin - BeautyViewSliderHeight,
                                   CGRectGetWidth(self.bounds) - 10 * BeautyViewMargin - BeautyViewSliderHeight, BeautyViewSliderHeight);
    self.sliderValueLabel.frame = CGRectMake(self.slider.frame.size.width + self.slider.frame.origin.x + BeautyViewMargin, BeautyViewMargin, BeautyViewSliderHeight, BeautyViewSliderHeight);
    _bottomBackgroundView.frame = CGRectMake(0, CGRectGetMaxY(_menu.frame), CGRectGetWidth(self.bounds), CGRectGetHeight(self.bounds) - CGRectGetMaxY(_menu.frame));
}
...
- (void)menu:(TCMenuView *)menu didChangeToIndex:(NSInteger)menuIndex option:(NSInteger)optionIndex {
    ...
    switch (menuIndex) {
        ...
        case PanelMenuIndexGesture:
        case PanelMenuIndexCosmetic:
...
- (void)URLSession:(NSURLSession *)session downloadTask:(NSURLSessionDownloadTask *)downloadTask didFinishDownloadingToURL:(NSURL *)location {
    ...
    NSError *fsErr = nil;
    [[NSFileManager defaultManager] moveItemAtURL:location toURL:[NSURL fileURLWithPath:task.destPath] error:&fsErr];
    if (fsErr) {
        [self.pituDelegate onLoadPituFailed];
        return;
    }
...
- (id<TCMenuItem>)menu:(TCMenuView *)menu itemAtMenuIndex:(NSInteger)index optionIndex:(NSInteger)optionIndex {
