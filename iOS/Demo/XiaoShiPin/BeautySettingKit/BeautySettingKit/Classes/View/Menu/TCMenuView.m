...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<TCMenuViewDataSource>)dataSource {
    if (self = [super initWithFrame:frame]) {
        ...
        CGRect menuFrame                                   = CGRectMake(0, CGRectGetHeight(self.bounds) - BeautyViewCollectionHeight, CGRectGetWidth(self.bounds), BeautyViewCollectionHeight);
...
- (void)setSelectedOption:(NSInteger)optionIndex inMenu:(NSInteger)menuIndex {
    ...
    if (menuIndex == _menuIndex) {
        ...
        [self.optionsCollectionView selectItemAtIndexPath:indexPath animated:NO scrollPosition:UICollectionViewScrollPositionCenteredHorizontally];
    }
}
