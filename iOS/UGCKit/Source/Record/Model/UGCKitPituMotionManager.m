...
@implementation UGCKitPituMotionManager
{
    NSMutableDictionary<NSString *, UGCKitPituMotion *> *_map;
}

+ (instancetype)sharedInstance
{
    static UGCKitPituMotionManager *sharedInstance = nil;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstance = [[UGCKitPituMotionManager alloc] init];
    });
    ...
    if (self) {
        ...
        NSArray *(^generate)(NSArray *) = ^(NSArray *inputArray){
            ...
            for (NSArray *item in inputArray) {
                UGCKitPituMotion *address = [[UGCKitPituMotion alloc] initWithId:item[0] name:item[2] url:item[1]];
...
@implementation UGCKitPituMotion
