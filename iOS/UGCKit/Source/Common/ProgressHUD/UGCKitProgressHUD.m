...
#define UGCKitMainThreadAssert() NSAssert([NSThread isMainThread], @"UGCKitProgressHUD needs to be accessed on the main thread.");

CGFloat const UGCKitProgressMaxOffset = 1000000.f;

static const CGFloat UGCKitDefaultPadding = 4.f;
static const CGFloat UGCKitDefaultLabelFontSize = 16.f;
static const CGFloat UGCKitDefaultDetailsLabelFontSize = 12.f;


@interface UGCKitProgressHUD () {
    // Deprecated
...
@interface UGCKitProgressHUDRoundedButton : UIButton
@end


@implementation UGCKitProgressHUD

#pragma mark - Class methods

+ (instancetype)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    UGCKitProgressHUD *hud = [[self alloc] initWithView:view];
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
    UGCKitProgressHUD *hud = [self HUDForView:view];
    if (hud != nil) {
        hud.removeFromSuperViewOnHide = YES;
...
+ (UGCKitProgressHUD *)HUDForView:(UIView *)view {
    NSEnumerator *subviewsEnum = [view.subviews reverseObjectEnumerator];
    for (UIView *subview in subviewsEnum) {
        if ([subview isKindOfClass:self]) {
            UGCKitProgressHUD *hud = (UGCKitProgressHUD *)subview;
...
- (void)commonInit {
    ...
    _animationType = UGCKitProgressHUDAnimationFade;
    _mode = UGCKitProgressHUDModeIndeterminate;
    _margin = 20.0f;
    _opacity = 1.f;
...
- (void)animateIn:(BOOL)animatingIn withType:(UGCKitProgressHUDAnimation)type completion:(void(^)(BOOL finished))completion {
    // Automatically determine the correct zoom animation type
    if (type == UGCKitProgressHUDAnimationZoom) {
        type = animatingIn ? UGCKitProgressHUDAnimationZoomIn : UGCKitProgressHUDAnimationZoomOut;
    }
    ...
    if (animatingIn && bezelView.alpha == 0.f && type == UGCKitProgressHUDAnimationZoomIn) {
        bezelView.transform = small;
    } else if (animatingIn && bezelView.alpha == 0.f && type == UGCKitProgressHUDAnimationZoomOut) {
    ...
    dispatch_block_t animations = ^{
        ...
        } else if (!animatingIn && type == UGCKitProgressHUDAnimationZoomIn) {
            bezelView.transform = large;
        } else if (!animatingIn && type == UGCKitProgressHUDAnimationZoomOut) {
            bezelView.transform = small;
        }
#pragma clang diagnostic push
#pragma clang diagnostic ignored "-Wdeprecated-declarations"
...
- (void)done {
    ...
    UGCKitProgressHUDCompletionBlock completionBlock = self.completionBlock;
    if (completionBlock) {
        completionBlock();
    }
    id<UGCKitProgressHUDDelegate> delegate = self.delegate;
...
- (void)setupViews {
    ...
    UGCKitBackgroundView *backgroundView = [[UGCKitBackgroundView alloc] initWithFrame:self.bounds];
    backgroundView.style = UGCKitProgressHUDBackgroundStyleSolidColor;
    ...
    UGCKitBackgroundView *bezelView = [UGCKitBackgroundView new];
    ...
    label.font = [UIFont boldSystemFontOfSize:UGCKitDefaultLabelFontSize];
    label.opaque = NO;
    ...
    detailsLabel.font = [UIFont boldSystemFontOfSize:UGCKitDefaultDetailsLabelFontSize];
    detailsLabel.opaque = NO;
    detailsLabel.backgroundColor = [UIColor clearColor];
    _detailsLabel = detailsLabel;

    UIButton *button = [UGCKitProgressHUDRoundedButton buttonWithType:UIButtonTypeCustom];
    button.titleLabel.textAlignment = NSTextAlignmentCenter;
    button.titleLabel.font = [UIFont boldSystemFontOfSize:UGCKitDefaultDetailsLabelFontSize];
...
- (void)updateIndicators {
    ...
    BOOL isRoundIndicator = [indicator isKindOfClass:[UGCKitRoundProgressView class]];

    UGCKitProgressHUDMode mode = self.mode;
    if (mode == UGCKitProgressHUDModeIndeterminate) {
    ...
    else if (mode == UGCKitProgressHUDModeDeterminateHorizontalBar) {
        // Update to bar determinate indicator
        [indicator removeFromSuperview];
        indicator = [[UGCKitBarProgressView alloc] init];
        [self.bezelView addSubview:indicator];
    }
    else if (mode == UGCKitProgressHUDModeDeterminate || mode == UGCKitProgressHUDModeAnnularDeterminate) {
        if (!isRoundIndicator) {
            // Update to determinante indicator
            [indicator removeFromSuperview];
            indicator = [[UGCKitRoundProgressView alloc] init];
            [self.bezelView addSubview:indicator];
        }
        if (mode == UGCKitProgressHUDModeAnnularDeterminate) {
            [(UGCKitRoundProgressView *)indicator setAnnular:YES];
        }
    }
    else if (mode == UGCKitProgressHUDModeCustomView && self.customView != indicator) {
        // Update custom view indicator
        [indicator removeFromSuperview];
        indicator = self.customView;
        [self.bezelView addSubview:indicator];
    }
    else if (mode == UGCKitProgressHUDModeText) {
        [indicator removeFromSuperview];
        indicator = nil;
    }
...
#pragma clang diagnostic pop
    ...
    if ([indicator isKindOfClass:[UIActivityIndicatorView class]]) {
        ...
        appearance = [UIActivityIndicatorView appearanceWhenContainedIn:[UGCKitProgressHUD class], nil];
#else
        // For iOS 9+
        appearance = [UIActivityIndicatorView appearanceWhenContainedInInstancesOfClasses:@[[UGCKitProgressHUD class]]];
#endif

        if (appearance.color == nil) {
            ((UIActivityIndicatorView *)indicator).color = color;
        }
    } else if ([indicator isKindOfClass:[UGCKitRoundProgressView class]]) {
        UGCKitRoundProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UGCKitRoundProgressView appearanceWhenContainedIn:[UGCKitProgressHUD class], nil];
#else
        appearance = [UGCKitRoundProgressView appearanceWhenContainedInInstancesOfClasses:@[[UGCKitProgressHUD class]]];
#endif
        if (appearance.progressTintColor == nil) {
            ((UGCKitRoundProgressView *)indicator).progressTintColor = color;
        }
        if (appearance.backgroundTintColor == nil) {
            ((UGCKitRoundProgressView *)indicator).backgroundTintColor = [color colorWithAlphaComponent:0.1];
        }
    } else if ([indicator isKindOfClass:[UGCKitBarProgressView class]]) {
        UGCKitBarProgressView *appearance = nil;
#if __IPHONE_OS_VERSION_MIN_REQUIRED < 90000
        appearance = [UGCKitBarProgressView appearanceWhenContainedIn:[UGCKitProgressHUD class], nil];
#else
        appearance = [UGCKitBarProgressView appearanceWhenContainedInInstancesOfClasses:@[[UGCKitProgressHUD class]]];
#endif
        if (appearance.progressColor == nil) {
            ((UGCKitBarProgressView *)indicator).progressColor = color;
        }
        if (appearance.lineColor == nil) {
            ((UGCKitBarProgressView *)indicator).lineColor = color;
        }
    } else {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
...
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 70000 || TARGET_OS_TV
    UGCKitBackgroundView *bezelView = self.bezelView;
...
- (void)updatePaddingConstraints {
    ...
    [self.paddingConstraints enumerateObjectsUsingBlock:^(NSLayoutConstraint *padding, NSUInteger idx, BOOL *stop) {
        ...
        padding.constant = (firstVisible && (secondVisible || hasVisibleAncestors)) ? UGCKitDefaultPadding : 0.f;
...
- (void)setMode:(UGCKitProgressHUDMode)mode {
    if (mode != _mode) {
        _mode = mode;
        [self updateIndicators];
    }
...
- (void)setCustomView:(UIView *)customView {
    if (customView != _customView) {
        ...
        if (self.mode == UGCKitProgressHUDModeCustomView) {
            [self updateIndicators];
        }
    }
}
...
@implementation UGCKitRoundProgressView
...
@implementation UGCKitBarProgressView
...
@interface UGCKitBackgroundView ()
...
@implementation UGCKitBackgroundView
...
- (instancetype)initWithFrame:(CGRect)frame {
    if ((self = [super initWithFrame:frame])) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_7_0) {
            _style = UGCKitProgressHUDBackgroundStyleBlur;
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
            ...
            _style = UGCKitProgressHUDBackgroundStyleSolidColor;
...
- (void)setStyle:(UGCKitProgressHUDBackgroundStyle)style {
    if (style == UGCKitProgressHUDBackgroundStyleBlur && kCFCoreFoundationVersionNumber < kCFCoreFoundationVersionNumber_iOS_7_0) {
        style = UGCKitProgressHUDBackgroundStyleSolidColor;
    }
    if (_style != style) {
        _style = style;
        [self updateForBackgroundStyle];
...
- (void)updateForBackgroundStyle {
    UGCKitProgressHUDBackgroundStyle style = self.style;
    if (style == UGCKitProgressHUDBackgroundStyleBlur) {
#if __IPHONE_OS_VERSION_MAX_ALLOWED >= 80000 || TARGET_OS_TV
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
...
- (void)updateViewsForColor:(UIColor *)color {
    if (self.style == UGCKitProgressHUDBackgroundStyleBlur) {
        if (kCFCoreFoundationVersionNumber >= kCFCoreFoundationVersionNumber_iOS_8_0) {
            self.backgroundColor = self.color;
        } else {
#if !TARGET_OS_TV
...
@implementation UGCKitProgressHUD (Deprecated)

#pragma mark - Class

+ (NSUInteger)hideAllHUDsForView:(UIView *)view animated:(BOOL)animated {
    NSArray *huds = [UGCKitProgressHUD allHUDsForView:view];
    for (UGCKitProgressHUD *hud in huds) {
        hud.removeFromSuperViewOnHide = YES;
        [hud hideAnimated:animated];
    }
    return [huds count];
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue completionBlock:(nullable UGCKitProgressHUDCompletionBlock)completion {
...
- (BOOL)dimBackground {
    UGCKitBackgroundView *backgroundView = self.backgroundView;
    UIColor *dimmedColor =  [UIColor colorWithWhite:0.f alpha:.2f];
    return backgroundView.style == UGCKitProgressHUDBackgroundStyleSolidColor && [backgroundView.color isEqual:dimmedColor];
}

- (void)setDimBackground:(BOOL)dimBackground {
    UGCKitMainThreadAssert();
    self.backgroundView.style = UGCKitProgressHUDBackgroundStyleSolidColor;
...
@implementation UGCKitProgressHUDRoundedButton
